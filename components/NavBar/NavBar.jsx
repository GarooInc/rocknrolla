  ...
  return (
                        ...
                        <li key={index}>
                            ...
                            <ul className="submenuitem shadow menu rounded-none bg-black text-white hover:bg-none hover:text-white">
                                {item.submenu.map((subitem, subindex) => (
                                    ...
                                    </li>{subindex < item.submenu.length - 1 && <div className='border-b border-white'></div>}
                                    </>
                                ))}
                            </ul>
                        </details>
    ...
    <div className="flex xl:hidden justify-between w-full">
         <div className="flex justify-between px-4 w-full">
            <Image src="/assets/images/logo.png" alt="Prodisa" className='cursor-pointer' width={60} height={60} onClick={() => router.push('/')} />
        ...
        {
            showMenu && (
                <ul className="menu menu-vertical bg-white text-blue-prod absolute top-0 right-0 z-10 h-screen w-80 p-6">
                        ...
                        {menu.map((item, index) => (
                            item.submenu ? (
                                <li key={index}>
                                    ...
                                    <ul className="shadow menu bg-white text-blue-prod">
                                        {item.submenu.map((subitem, subindex) => (
                                            <li key={subindex} className='nav-item'>
                                                <a href={subitem.link}>{subitem.name}</a>
                                            </li>
